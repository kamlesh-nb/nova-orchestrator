...
was removed from the control plane 2026-09-03, see `src/store/httpconfig.ky` + `src/artifacts/cfgstore.ky`).
...
# entrypoint directly, e.g.  kyte bin/service.ky -o build\release\bin\service.exe --release
```

Both test runners must find **`kyte(.exe)` on PATH** and run from a checkout laid out beside the `lang`
toolchain (the resolver finds this package via `../packages`, and the stdlib resolves relative to the
`lang/` CWD, which the scripts `cd` into). `kyte` must be a **ReleaseFast** build: a Debug kyte's leak
gate exits 1 on every `kyte test`, so the suite would report red while printing "0 failed"
...
toolchain, with a ReleaseFast `kyte` on PATH.
...
- `src/net/` — `proxy.ky` (LB pool + the fd-handoff serve loop), `service.ky`, `autoscale.ky`,
  `netns.ky`.
- `src/orch/` — `manifest.ky`, `supervisor.ky`, `rollout.ky`, `lease.ky`/`asynclease.ky`
  (HA leader lease), `health.ky`, `spec.ky`, `membership.ky`, `autoscaler.ky`, `isolation.ky`.
- `src/store/` — the config store: `config.ky` (in-memory core + CAS), `wire.ky` (HTTP codec),
  `httpconfig.ky` (async client over artifactd's `/cfg/*`). artifactd hosts it via `src/artifacts/cfgstore.ky`.
...
file descriptor to the app** — `os.socket.sendFd(ctrl, clientFd, payload)` (`src/net/proxy.ky`
~L906), which on POSIX sends the fd as `SCM_RIGHTS` ancillary data over the AF_UNIX channel
(`lang/src/lib/std/os/posix/socket.ky`). The app then owns the socket and replies to the client
...
- The path is derived identically in `bin/service.ky`, `src/orch/spec.ky`, and
  `src/orch/manifest.ky` (from the workload name), and `KYTE_HANDOFF_SOCK` overrides it — service and
...
Today `lang/src/lib/std/os/windows/socket.ky` `sendFd`/`recvFd` are **stubs returning `-1`**: the
...
1. **Control channel:** replace the AF_UNIX rendezvous with a **named pipe** (`\\.\pipe\kyte-<name>`)
   or loopback TCP — the rendezvous "path" becomes a pipe name, not a filesystem path, so the
   derivation in `spec.ky`/`manifest.ky`/`service.ky` must be **target-conditional** (short
   `/tmp/*.sock` on POSIX, `\\.\pipe\kyte-*` on Windows).
...
`src/net/proxy.ky` on the target OS. Track: `docs/platform-readiness.md` row **C-Iso** ("confirm
