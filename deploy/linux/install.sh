#!/usr/bin/env bash
# Install the Kyte orchestrator daemons (artifactd, kynatord, service) as systemd units on Linux.
#
# Layout (override with env vars):
#   BIN_DIR   /opt/nova-orchestrator/bin      built binaries are copied here
#   CONF_DIR  /etc/nova-orchestrator          kynatord.json / service.json / artifactd.env
...
#   --from   directory containing service/kynatord/kynatorctl/artifactd (default: ../../build/release/bin)
...
install -m 0644 "$here/kyte-artifactd.service" "$here/kyte-kynatord.service" "$here/kyte-service.service" /etc/systemd/system/
...
if [ "$DO_START" = 1 ]; then
  ...
  systemctl --no-pager status kyte-artifactd.service kyte-kynatord.service kyte-service.service | sed -n '1,30p' || true
fi

echo "Done. Manage with: systemctl {status,start,stop,restart} kyte-{artifactd,kynatord,service}"
