#!/usr/bin/env bash
# Install the Kyte orchestrator daemons (artifactd, kynatord, service) as launchd system daemons on macOS.
#
...
#   --from  directory with service/kynatord/kynatorctl/artifactd (default: ../../build/release/bin)
...
for label in com.kyte.artifactd com.kyte.kynatord com.kyte.service; do
...
if [ "$DO_LOAD" = 1 ]; then
  ...
  launchctl print system/com.kyte.kynatord 2>/dev/null | sed -n '1,12p' || true
fi

echo "Done. Manage with: sudo launchctl {bootstrap,bootout} system /Library/LaunchDaemons/com.kyte.<c>.plist"
