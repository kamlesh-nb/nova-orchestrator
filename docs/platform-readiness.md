# Kyte platform readiness: medium-scale production plan
...
architecture: single-threaded Kyte replicas scaled horizontally, with the orchestrator as the control and data
plane that spreads traffic across them.

## The architecture this plan assumes

Kyte is single-threaded on purpose, to avoid synchronisation cost. One reactor core already beats Rust axum
...
|----|------|--------|----------|--------|
| T1-1 | Per-workload rolling update (surge/maxUnavailable) | C-T1-1 | P0 | done (`src/orch/nativelet.ky:328-349` Job roll state machine + `src/orch/supervisor.ky` setSpec/rollOne: one replica swapped per grace window, N-1 keep serving) |
| T1-2 | Readiness gate before a replica takes traffic | C-T1-2 | P0 | done (`src/net/proxy.ky:222-230`: under health checks a new backend starts DRAINED and enters rotation only after `hcRise` consecutive OK probes) |
| T1-3 | Real timed graceful drain (drain-from-LB then kill) | C-T1-3 | P0 | done (`src/orch/supervisor.ky:202-220` stopProc: SIGTERM, wall-clock deadline poll, SIGKILL on overrun; LB-remove-first folds into T1-1) |
| T1-4 | Torn-free atomic discovery file write | C-T1-4 | P0 | done (`src/orch/nativelet.ky:35-39` atomicWriteText used at `:159`, and `src/net/service.ky:103-108`: temp file + atomic rename) |
...
then a fresh supervisor `start()` (`src/orch/nativelet.ky:303-310`), so every replica of that workload goes
down together. For a horizontal-scaling platform this is the single most important missing piece.

- Add `maxSurge` / `maxUnavailable` to the spec (`src/orch/spec.ky`, and the manifest schema in
  `src/orch/manifest.ky`), defaulting to a safe rolling policy (surge 1, unavailable 0).
- Change the spec-changed branch in `reconcileScan` to roll replicas in batches: bring up new-spec replicas up
  ...
  (`src/orch/supervisor.ky:216-255`), so this is a per-replica state machine on top of it, not a rewrite.
...
after spawn with no readiness check (`src/net/autoscale.ky:143`), so traffic routes to a not-yet-bound port
...
- A replica must pass at least one readiness probe before the data plane routes to it. Reuse the existing
  probe machinery (`httpProbe` in `src/orch/nativelet.ky:427-440`, and the proxy's active health checks in
  `src/net/proxy.ky:620-679`) rather than adding a second one.
- In `net/autoscale.ky`, do not set `up = 1` on spawn; let the first successful health check flip it (the
...
(`src/orch/supervisor.ky:165-175`) spins `while (tries < 50)` over a non-blocking `tryWait` with no sleep, so
...
- Order drain correctly on scale-down and rolling deploy: first remove the replica from the data plane (mark
  ...
  (`src/net/autoscale.ky:149`); extend the same ordering to workload replicas.
- Move the autoscaler's blocking `p.wait()` off the reactor (`src/net/autoscale.ky:155`) to the async reap,
...
atomic. `ServiceRegistry.flush` (`src/net/service.ky:93-103`) rewrites the whole file via `writeText` on
...
- Write to a temp file then rename over the target (atomic on POSIX), so a reader always sees a complete
  generation. `Nativelet.publishDiscovery` (`src/orch/nativelet.ky:452-459`) and the resolver
  (`src/net/service.ky:108-143`) are the two ends to keep in step.
...
`coroStart(handleClient(...))` per accept with no cap (`src/net/proxy.ky:714`), each client allocates a 64 KB
...
- Reap idle backend fds on drain: when a slot goes `up = 0`, close its pooled idle fds
  (`b.idle[tid]`, `src/net/proxy.ky:534`) instead of leaving them to accumulate; add a simple idle-timeout
  reaper.
...
`SqlConfigStore.casBy` (`src/store/sqlconfig.ky:167-205`) is a read-then-write across two awaits, so two
...
- Add a gating live test that races two `AsyncLeaderLease.tryAcquire` calls against a real NovaDB and asserts
  exactly one wins. The current HA property tests (`tests/198_ha_cluster.ky`) run over one shared in-memory
...
- **Isolation-downgrade observability.** `isolation.applyLimits` (`src/orch/isolation.ky:31-49`) ignores every
...
- **Autoscaler metric.** `tickCpu` (`src/orch/autoscaler.ky:62-74`) feeds aggregate cgroup CPU into a PID
  ...
  (`src/orch/isolation.ky:36-48`).
- **Config loud-fail.** The load path (`src/cfg/config.ky:98-128, 251-269`) uses `?? default` on every field,
...
- **Output escaping.** `renderMetrics` / `appsTable` (`src/orch/health.ky:113`,
  `src/orch/controlplane.ky:120`) do not escape names, so a name with a quote breaks Prometheus exposition or
  the HTML. `backup.dump/restore` (`src/orch/backup.ky`) assumes tab/newline-free values; add escaping.
- **Probe robustness.** `httpProbe` (`src/orch/nativelet.ky:427-440`) reads the status byte at fixed offset 9
  assuming exactly `"HTTP/1.1 "`; parse the status line properly so an `HTTP/1.0` reply is not misread. Add
  backoff to `probeAndHeal` (`src/orch/nativelet.ky:412-416`) so a persistently failing probe does not become
  a tight restart loop.
- **Proxy streaming ceiling.** `readMessage` (`src/net/proxy.ky:427`) truncates response bodies over 64 KB;
...
- **Service VIP bind.** `proxyAcceptLoop` binds `INADDR_ANY` and discards the `host` argument
  (`src/net/proxy.ky:684-701`) while `serveAddr` advertises per-VIP binding. At medium scale you front
...
Kyte replicas on at medium scale."
