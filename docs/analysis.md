...
Kyte across `src/net`, `src/orch`, `src/store`, `src/cfg`, `src/os`, the four `bin/` entrypoints, the
...
1. **The live-path CAS is not atomic, so leader election can split-brain. CRITICAL.**
   The entire HA safety argument (`orch/lease.ky:5-10`, `orch/asynclease.ky`) rests on "CAS = exactly one
   winner per epoch." The in-memory `ConfigStore.casBy` (`store/config.ky:129-143`) is correctly atomic (a
   synchronous index check then write), which is why the offline chaos tests pass. But the live
   `SqlConfigStore.casBy` (`store/sqlconfig.ky:167-205`) is a read-then-write across two awaits:
...
3. **HA is a single-store lease, not consensus. HIGH (framing).**
   ...
   NovaDB (`store/sqlconfig.ky:3-8`). That is a legitimate design (etcd-style lease over a consistent store),
   but the doc-comments oversell it as fencing-grade distributed safety. Orchestrator HA is only ever as strong
   as NovaDB's own leader/follower durability (`SET DURABLE COMMIT`, `store/sqlconfig.ky:70-79`) plus a
   correct atomic acquire. State that plainly.

4. **Service VIP bind is silently non-functional. HIGH.**
   `net/proxy.ky:684-701` (`proxyAcceptLoop`) always binds `INADDR_ANY` via `sys.makeSockaddrIn(port, 0)` and
   ...
   and `net/service.ky` depends on it. Net effect: every Service with a VIP binds all interfaces on that port,
...
1. **Graceful shutdown has effectively no grace window. HIGH.**
   `Supervisor.stopProc` (`orch/supervisor.ky:165-175`) sends SIGTERM then spins `while (tries < 50)` over a
...
2. **A manifest change bounces the whole workload, no rolling update. HIGH.**
   `reconcileScan` (`orch/nativelet.ky:303-310`) reacts to any spec change with `stopAll()` then a fresh
   supervisor `start()`, so every replica goes down together and then comes back. There is no surge /
   maxUnavailable / rolling replacement for workload replicas. `orch/rollout.ky` is only for rolling
...
3. **Probe-driven heal is a stop-the-world bounce with no backoff. MEDIUM.**
   At `PROBE_FAIL_LIMIT`, `probeAndHeal` (`orch/nativelet.ky:412-416`) does `stopAll()` then `start()`,
   ...
   (`orch/alerts.ky:60`) but the action does not.

4. **The autoscaler regulates aggregate cgroup CPU against a per-workload setpoint. MEDIUM.**
   `isolation.cgroupDir` keys on `sp.name` only (`orch/isolation.ky:21-22`), so all replicas of a workload
   share one cgroup and `cpuUsageUsec` returns aggregate CPU across replicas. `tickCpu`
   (`orch/autoscaler.ky:62-74`) feeds that aggregate into a PID with a fixed percent setpoint, but aggregate
   ...
   (`orch/isolation.ky:36-48`), which is probably not what a user expects from "cpuMilli per workload."

5. **Blocking reaps inside the reactor-0 control coroutine. MEDIUM.**
   The autoscaler's scale-down calls `p.terminate()` then a synchronous `p.wait()` (`net/autoscale.ky:155`)
   on the same reactor that runs the health sweep; a backend slow to exit on SIGTERM stalls reactor 0.
   `netns.runIp` (`net/netns.ky:109-112`) likewise does a blocking `pr.wait()` with no timeout.

6. **Data-plane resource and lifecycle gaps. MEDIUM.**
   `net/proxy.ky` has a hard 64 KB per-message buffer (`readMessage`, `:427`): oversized bodies set
...
7. **Non-atomic discovery file writes. MEDIUM.**
   `ServiceRegistry.flush` (`net/service.ky:93-103`) rewrites the whole `name=host:port` file via `writeText`
...
8. **Silent security-relevant isolation downgrades. MEDIUM.**
   `isolation.applyLimits` (`orch/isolation.ky:31-49`) ignores every `writeText` result, so on macOS or
   unprivileged Linux the workload runs unlimited with no warning or metric. netns and sandbox degrade to a
   plain spawn off Linux (`orch/supervisor.ky:121-123, 150-156, 197`). This is correct by design, but there
...
9. **Smaller correctness notes. LOW.**
   `httpProbe` (`orch/nativelet.ky:427-440`) reads the status byte at fixed offset 9, assuming exactly
   `"HTTP/1.1 "`; an `HTTP/1.0` reply or any deviation misreads it, and only the first digit is checked.
   `backup.dump/restore` (`orch/backup.ky`) assumes values are tab/newline-free with no escaping, so a spec
   or lease value containing a newline corrupts the dump. `renderMetrics`/`appsTable`
   (`orch/health.ky:113`, `orch/controlplane.ky:120`) do not escape workload or app names, so a name with a
   quote breaks Prometheus exposition or the HTML. The unbounded `10.66.${idx}.0/30` netns addressing
   (`net/netns.ky:55`) produces invalid addresses beyond idx 255 with no check.

## Part 3: what is genuinely production-quality

This is a real system, and these parts are good.

- **The reconcile loop** (`orch/nativelet.ky:286-332`). The desired-vs-actual diff is correct and complete
...
- **The supervisor** (`orch/supervisor.ky:216-255`). Non-blocking `tryWait` throughout, correct in-place list
...
- **The fencing-epoch design** (`orch/lease.ky`, `orch/health.ky:124-129`). This is the right model:
...
- **The data plane** (`net/proxy.ky`). Four LB strategies, per-reactor lock-free cursors and connection
...
- **Observability** (`orch/health.ky`, `orch/alerts.ky`). healthz/readyz/metrics, replication-lag clamp,
  fenced-role readiness handling, and Prometheus plus log alert rendering with severities. Thorough and usable.

- **The PID autoscaler math** (`net/autoscale.ky:70-77`). Textbook PID with anti-windup integral clamp,
...
- **The webui** (`webui/src/`). A real, fully-wired `kyte init web` vertical-slice app, not a skeleton. All six
...
- `ConfigStore` (`store/config.ky`) is the in-memory reference/oracle: keys in a `List` with linear-scan
...
- `SqlConfigStore` (`store/sqlconfig.ky`) is the real path over the async `Connection` seam against NovaDB.
  ...
  Kyte-side prefix filter (fine for a small config store, and independent of which SQL features NovaDB
...
- HA/lease/reconcile are genuinely tested at the logic level, not smoke-tested: `188_leader_lease.ky` and
  `198_ha_cluster.ky` are real property tests (fencing-after-promotion, clock-skew safety, N-node election,
...
- `SqlConfigStore` is tested offline only via a hand-written `FakeConn` (`185_sqlconfig.ky`) that simulates
