...
| P6 Security (authn/authz + mTLS) | PARTIAL | (1) REPLICA AUTHZ: HMAC challenge-response on the replication stream (wrong/missing key refused before any frame). (2) CONFIG-API RBAC (btree tier): executor enforces users/roles/per-object perms + no-token rejection; NEW adminGate requires an ADMIN session for the SET FENCE EPOCH / SET DURABLE COMMIT control commands. btree 36/36 | mutual-TLS on replication + client hops; thread login/token through the Nova db.Connection seam so the store authenticates (driver follow-on) |
