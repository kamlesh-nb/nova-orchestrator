...
| P7 Operability (backup/PITR, upgrade, runbooks) | PARTIAL | FULL backup/restore DONE: btree exportSnapshot (page image + WAL) + restoreSnapshot; round-trips byte-for-byte + MVCC-visible after WAL recover; test "P7 backup/restore" 38/38 | PITR replay-to-intermediate-seq (needs open-without-recover + WAL archive); rolling upgrade w/ rollback; membership add/remove; runbook per failure mode |
| P8 Chaos + soak (RPO/RTO proof) | PARTIAL | chaos_suite.sh reports numbers: kill-leader-after-load + kill-leader-mid-write 2/2 PASS; in-process: corrupt-frame-rejected (NEW test), fenced-old-leader, follower-reship-after-crash. STATED RPO=0 (sync/quorum), RTO=lease TTL + tick | subprocess kill-follower-mid-apply + partition live harness; disk-full + clock-skew injection; multi-hour soak in CI |
