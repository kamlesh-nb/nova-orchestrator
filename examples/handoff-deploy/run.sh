...
# derives the rendezvous /tmp/nova-<name>.sock, starts the SERVICE (gateway) with NOVA_HANDOFF_SOCK +
# NOVA_PORT, starts N APP replicas with NOVA_HANDOFF_SOCK, and curls the front port. The Nova equivalent
# — via supervisors + manifest.serviceSpecFor/toSpec — is in deploy.nova (what orchd runs); it sets the
...
NAME=shop; SOCK="/tmp/nova-$NAME.sock"; PORT=8140
...
NOVA_HANDOFF_SOCK="$SOCK" NOVA_PORT="$PORT" "$SVC" >/tmp/nd_svc.log 2>&1 &
NOVA_HANDOFF_SOCK="$SOCK" "$APP" >/tmp/nd_a1.log 2>&1 &
NOVA_HANDOFF_SOCK="$SOCK" "$APP" >/tmp/nd_a2.log 2>&1 &
sleep 2
