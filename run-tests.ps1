<#
.SYNOPSIS
  Run every nova-orchestrator test via `nova test` on Windows (PowerShell mirror of run-tests.sh).

.DESCRIPTION
  The Nova resolver finds this package's modules through ../packages, so this must run from a checkout
  laid out beside the `lang` toolchain, with nova.exe on PATH. We invoke from the lang/ directory so the
  stdlib resolves relative to CWD -- exactly like run-tests.sh does.

  Requirements / caveats:
    * nova.exe must be a ReleaseFast build. A Debug nova's leak gate exits 1 on every `nova test`, so the
...
if (-not (Test-Path $lang)) { Write-Error "expected the Nova toolchain at $lang"; exit 1 }
if (-not (Get-Command nova -ErrorAction SilentlyContinue) -and -not (Get-Command nova.exe -ErrorAction SilentlyContinue)) {
  Write-Error "nova(.exe) not found on PATH (install the toolchain; see lang/CLAUDE.md)"; exit 1
}

Push-Location $lang
try {
  $files = @()
  $files += Get-ChildItem -Path (Join-Path $here "tests") -Filter *.nova -File -ErrorAction SilentlyContinue
  $wf = Join-Path $here "webui\tests\features"
  if (Test-Path $wf) { $files += Get-ChildItem -Path $wf -Filter *.nova -File -ErrorAction SilentlyContinue }
  if ($Filter) { $files = $files | Where-Object { $_.Name -like "*$Filter*" } }

  $pass = 0; $fail = 0
  foreach ($t in $files) {
    $out = & nova test $t.FullName 2>&1
